...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Qeydiyyat uğurludur! İndi giriş edə bilərsiniz." });
    } catch (err) {
        res.status(500).json({ error: "İstifadəçi mövcuddur və ya verilənlər bazası xətası." });
    }
});
...
app.post('/login', async (req, res) => {
    const { username, password } = req.body;
    try {
        // 1. TƏHLÜKƏLİ SORĞU (Hücum üçün bura belə qalmalıdır)
        const query = `SELECT * FROM users WHERE username = '${username}'`; 
        const userResult = await pool.query(query);
        ...
        if (userResult.rows.length > 0) {
            ...
            /* const isMatch = await bcrypt.compare(password, user.password);
            if (!isMatch) return res.status(401).json({ error: "Şifrə yanlışdır!" });
            */

            // ✅ BİRBAŞA CAVAB QAYTARIR (Hücumun uğurlu olduğunu görmək üçün):
            res.status(200).json({ 
                message: "Giriş uğurla tamamlandı! (SQL Injection uğurlu)", 
